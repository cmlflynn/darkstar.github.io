...
def calculate_horta2021_halo():
    ...
    # Model: TRI-CUT-DISC (Triaxial Single Power Law for the total halo)
    alpha = 3.5
    # The paper uses a cut-off but the field halo fit is often quoted as alpha~3.5
    # We will implement it as a Single Power Law for the comparison compendium
    ...
    L_total = 4 * np.pi * p * q * rho_0 * (int0 + int1)

    print(f"--- Horta et al. (2021) - The contribution of N-rich stars to the Galactic stellar halo ---")
    ...
    plt.loglog(r_vals, rho_vals / (1000**3), color='crimson', linewidth=2, label='Horta et al. (2021) TRI-CUT Model (1 kpc Core)')
    ...
    plt.title('Stellar Halo Density Profile (Horta et al. 2021)', fontsize=14)
    plt.xlabel('Radius $r$ [kpc]', fontsize=12)
    plt.ylabel('Luminosity Density [$L_\\odot/pc^3$]', fontsize=12)
    plt.legend(frameon=True)
    plt.grid(True, which="both", ls="-", alpha=0.2)
    
    plt.savefig('assets/horta2021_density_profile.png', dpi=300)
    print("Plot saved to: assets/horta2021_density_profile.png")
