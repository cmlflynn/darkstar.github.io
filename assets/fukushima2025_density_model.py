...
def calculate_fukushima2025_halo():
    # Parameters from Fukushima et al. (2025) - PASJ 77, 178
    # Model: Broken Power Law (BHB stars from Subaru HSC)
    alpha_in = 3.90
    alpha_out = 9.1
    r_break = 184.0
    r_core = 1.0   # 1 kpc constant density core (project requirement)
    q = 1.56       # Prolate flattening
    R_sun = 8.275 
    ...
    print(f"--- Fukushima et al. (2025) Subaru HSC BHB Halo Properties ---")
    ...
    # Plotting
    r_vals = np.logspace(-0.5, 2.8, 500)
    rho_vals = np.array([rho_0 * profile(r) for r in r_vals])

    plt.figure(figsize=(10, 7))
    plt.axvspan(36, 575, color='lightblue', alpha=0.3, label='Valid Data Range (36-575 kpc)')
    plt.loglog(r_vals, rho_vals / (1000**3), color='darkred', linewidth=2, label='Fukushima et al. (2025) Model (1 kpc Core)')
    ...
    plt.title('Milky Way Stellar Halo Density Profile (Fukushima et al. 2025)', fontsize=14)
    plt.xlabel('Radius $r$ [kpc]', fontsize=12)
    plt.ylabel('Luminosity Density [$L_\\odot/pc^3$]', fontsize=12)
    plt.legend(frameon=True)
    plt.grid(True, which="both", ls="-", alpha=0.2)
    
    plt.savefig('assets/fukushima2025_density_profile.png', dpi=300)
    print("Plot saved to: assets/fukushima2025_density_profile.png")

if __name__ == "__main__":
    calculate_fukushima2025_halo()
