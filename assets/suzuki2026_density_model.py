...
def calculate_suzuki2026_halo():
    ...
    print(f"--- Suzuki et al. (2026) MSTO Halo Properties ---")
    ...
    plt.loglog(r_vals, rho_vals / (1000**3), color='darkolivegreen', linewidth=2, label='Suzuki et al. (2026) Model (1 kpc Core)')
    ...
    plt.title('Milky Way Stellar Halo Density Profile (Suzuki et al. 2026)', fontsize=14)
    plt.xlabel('Radius $r$ [kpc]', fontsize=12)
    plt.ylabel('Luminosity Density [$L_\\odot/pc^3$]', fontsize=12)
    plt.legend(frameon=True)
    plt.grid(True, which="both", ls="-", alpha=0.2)
    
    plt.savefig('assets/suzuki2026_density_profile.png', dpi=300)
    print("Plot saved to: assets/suzuki2026_density_profile.png")

if __name__ == "__main__":
    calculate_suzuki2026_halo()
