...
def calculate_kurbatov2024_halo():
    ...
    print(f"--- Kurbatov et al. (2024) Metal-poor RGB Halo Properties ---")
    ...
    plt.axvspan(1, 18, color='lightblue', alpha=0.3, label='Valid Data Range (1-18 kpc)')
    plt.loglog(r_vals, rho_vals / (1000**3), color='orangered', linewidth=2, label='Kurbatov et al. (2024) Model (1 kpc Core)')
    ...
    plt.title('Milky Way Stellar Halo Density Profile (Kurbatov et al. 2024)', fontsize=14)
    plt.xlabel('Radius $r$ [kpc]', fontsize=12)
    plt.ylabel('Luminosity Density [$L_\\odot/pc^3$]', fontsize=12)
    plt.legend(frameon=True)
    plt.grid(True, which="both", ls="-", alpha=0.2)
    
    plt.savefig('assets/kurbatov2024_density_profile.png', dpi=300)
    print("Plot saved to: assets/kurbatov2024_density_profile.png")

if __name__ == "__main__":
    calculate_kurbatov2024_halo()
