...
def calculate_lopezcorredoira2024_halo():
    # Parameters from Lopez-Corredoira et al. (2024) - A&A 684, A135
    # Model: Single Power Law for the Outer Halo (25-90 kpc)
    alpha = 4.6    # Fit for K-giants
    ...
    print(f"--- Lopez-Corredoira et al. (2024) K/M Giant Halo Properties ---")
    ...
    plt.loglog(r_vals, rho_vals / (1000**3), color='forestgreen', linewidth=2, label='Lopez-Corredoira et al. (2024) Model (1 kpc Core)')
    ...
    plt.title('Milky Way Stellar Halo Density Profile (Lopez-Corredoira et al. 2024)', fontsize=14)
    plt.xlabel('Radius $r$ [kpc]', fontsize=12)
    plt.ylabel('Luminosity Density [$L_\\odot/pc^3$]', fontsize=12)
    plt.legend(frameon=True)
    plt.grid(True, which="both", ls="-", alpha=0.2)
    
    plt.savefig('assets/lopezcorredoira2024_density_profile.png', dpi=300)
    print("Plot saved to: assets/lopezcorredoira2024_density_profile.png")

if __name__ == "__main__":
    calculate_lopezcorredoira2024_halo()
